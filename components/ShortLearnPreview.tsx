...
export default function SkillShortPreview() {
  ...
  const fetchReels = useCallback(async () => {
    try {
      ...
      // ✅ FILTER ONLY NON-BATTLE REELS (Learning content)
      const learningReels = data.filter(
        (item: any) => item.postType === "reel" && item.isSkillBattle !== true
      );

      setReels(learningReels.slice(0, 10));
    } catch (error) {
