...
const ContestItem = ({ item, userId }: any) => {
  ...
  return (
    <View style={{ padding: 12, backgroundColor: "#fff", marginBottom: 10, borderRadius: 10 }}>
      <Text>{item.title}</Text>

      {item.status === "active" ? (
        <TouchableOpacity onPress={handleJoin}>
          <Text style={{ color: "green", marginTop: 5 }}>Participate Now</Text>
        </TouchableOpacity>
      ) : (
        <Text style={{ color: "gray", marginTop: 5 }}>Not Active</Text>
      )}
    </View>
  );
};
